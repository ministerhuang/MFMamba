...
class Trainer:
    ...
    def train(self,
        ...
        if self.env_type == "pytorch":
            ...
            if self.model_mae is not None:
                self.model_mae.cuda(self.local_rank)
                self.model_mae = torch.nn.SyncBatchNorm.convert_sync_batchnorm(self.model_mae)
                self.model_mae = torch.nn.parallel.DistributedDataParallel(
                    self.model_mae,
                    device_ids=[self.local_rank],
                    output_device=self.local_rank,
                    find_unused_parameters=True
        ...
        # Scheduler setup for optimizer
        warmup_steps = int(self.max_steps * (self.warmup if self.warmup != 0.0 else 0.1))
        def init_scheduler(optimizer, name):
            if self.scheduler_type == "cosine_with_warmup":
                return get_cosine_schedule_with_warmup(optimizer, warmup_steps, self.max_steps)
                ...
                return get_polynomial_decay_schedule_with_warmup(optimizer, warmup_steps, self.max_steps)
            elif self.scheduler_type == "poly":
                from light_training.utils.lr_scheduler import PolyLRScheduler
                lr = optimizer.state_dict()['param_groups'][0]['lr']
                print(f"initial lr for {name} is {lr}")
                return PolyLRScheduler(optimizer, initial_lr=lr, max_steps=self.max_steps)
            return None

        self.scheduler = init_scheduler(self.optimizer, "optimizer")
        self.scheduler_mae = init_scheduler(self.optimizer_mae, "optimizer_mae")

        if "poly" in self.scheduler_type:
            print(f"scheduler_type is {self.scheduler_type}, warmup steps is {warmup_steps if 'warmup' in self.scheduler_type else 0}")
        ...
        with tqdm(total=self.num_step_per_epoch, disable=(self.local_rank != 0)) as t:
            for i in range(self.num_step_per_epoch):
                ...
                if self.auto_optim:
                    ...
                    for param in self.model_mae.parameters(): param.grad = None

                if not self.auto_optim:
                    loss = self.training_step(batch, self.batch_size)
                else:
                    ...
                    if self.grad_scaler is not None:
                        ...
                        self.grad_scaler.unscale_(self.optimizer_mae)

                        torch.nn.utils.clip_grad_norm_(self.model.parameters(), 12)
                        torch.nn.utils.clip_grad_norm_(self.model_mae.parameters(), 12)

                        self.grad_scaler.step(self.optimizer)
                        self.grad_scaler.step(self.optimizer_mae)
                    ...
                    if self.scheduler is not None:
                        self.scheduler.step()
                    if self.scheduler_mae is not None:
                        self.scheduler_mae.step()

                    # Log learning rates
                    lr_main = self.optimizer.state_dict()['param_groups'][0]['lr']
                    lr_mae = self.optimizer_mae.state_dict()['param_groups'][0]['lr']
                    self.log("lr_main", lr_main, self.global_step)
                    self.log("lr_mae", lr_mae, self.global_step)

                    t.set_postfix(loss=loss.item(), lr=lr_main)
