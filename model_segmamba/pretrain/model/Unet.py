...
import random 
from model_segmamba.pretrain.model.models import Model
from model_segmamba.pretrain.dataset.image_utils import pad_or_crop_image
...
class Unet_missing(nn.Module):
    ...
    def __init__(self, input_shape, in_channels=4, out_channels=4, init_channels=16, p=0.2, pre_train = False, deep_supervised = False, mdp = 0, mask_modal = [], patch_shape = 128,  mask_ratio = 0.875, augment = False, batch_size = 2):
    ...
    def forward(self, x, location = None, fmdp = None, aug_choises = None, batch_size = 2):
        ...
        if self.pre_train and location == None: # never into this branch
            ...
            if fmdp == None:              # in finetune, the x inputed is already do the data augmentation and masking. so no need mask again
                ...
                for ci in range(batch_size):
                    ...
                    all_mask_modal.append(cindex)

                    mask_np = MaskEmbeeding2(1, raw_input=self.raw_input, mdp=1, mask=False, patch_shape=128, mask_modal = cmask_modal)
                    
                    mask = mask_np.to(x.device).float()
                    pl = location[ci]
                    zmin, ymin, xmin = pl[0]
                    zmax, ymax, xmax = pl[1]
                    ...
                    x[ci] = inputs_S1[ci] * mask + limage_crop * (1 - mask)
        else:
            mask = torch.ones_like(x)
        ...
        if self.training:
            return uout, mask.sum((2,3,4)), style, content, all_mask_modal  #返回一个[]，里面存两个batch分别遮掩的模态
        else:
            return uout
